...
| Password show/hide eye | `.toggle-sensitive-btn .icon` | Moodle's password-unmask toggle (`<button class="toggle-sensitive-btn btn btn-secondary"><i class="icon fa-regular fa-eye">`, in `.toggle-sensitive-wrapper.input-group`). FontAwesome, so `color:` is the lever. **Dark themes (#153):** the generic dark `.icon, .fa { color: #1d2125 !important }` painted it dark-on-dark on the dark login card → invisible. Re-light to `${tokens.bodyText}` (CFA Light Grey `#F0EEEE`). Appended to the existing login-dark block (`body#page-login-index …`, gated on `isDarkBg(tokens.loginBg)`), so it tracks the login page's own dark handling exactly. `body#page-login-index .toggle-sensitive-btn .icon` (1,2,0)+`!important` beats the generic `.icon` (0,1,0). The signup page (`#page-login-signup`) renders the password field WITHOUT the eye toggle, so no fix needed there. #153 |
...
| Quiz "Time limit" dialog (dark surface) | `.mod_quiz_preflight_popup` → `.moodle-dialogue-wrap` (bg+border), `.moodle-dialogue-hd` (border), hd/bd/`.closebutton` (text), `.moodle-dialogue-bd a` (link) | The SAME YUI `M.core.dialogue` as the Cancel-button row above (`.mod_quiz_preflight_popup`, portalled to `<body>`). The whole dialog rendered WHITE on dark themes — Moodle `core.scss` paints `.moodle-dialogue-wrap { background: $white }` + header `border-bottom: 1px solid #dee2e6`, and the generator deliberately keeps generic `.moodle-dialogue-bd` white-with-dark-text. User asked to darken it to the CFA design pattern → mirror the #142 move-dialog: repaint the wrap `cardBg` + `cardBorder`, override the hd border, flip header/body/close-`×` text to `bodyText` (light), links `linkColour`. **Buttons preserved:** the body text sweep uses `*:not(.btn):not(button):not(input):not(a)` so the green "Start attempt" (Buttons section) and red "Cancel" (#129) keep their own colours. Scoped to the unique `.mod_quiz_preflight_popup` (other YUI dialogs stay white); `(0,2,0)`+`!important` beats the generic dialog rules + Moodle's un-`!important` white. Dark presets only. #154 |
