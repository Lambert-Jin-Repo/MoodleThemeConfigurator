...
**Newly reported (handled separately, next):** on dark themes the pre-existing page-wrapper rule `#page, #page-wrapper, … { background-color: ${tokens.pageBg} !important }` (`scss-generator.ts:482`) paints the content wrapper solid, covering Moodle's `body`-level background image (set via Boost's "Background image" setting) — so the image is hidden behind the dark page. NOT caused by the quiz-edit work (verified: every background it adds is `#page-mod-quiz-edit`-scoped). Planned fix: make the page wrapper transparent with `pageBg` kept as a `body`-level fallback; cards/nav/drawers stay opaque for readability. Tracked as a separate issue with its own research + verify cycle.
