...
- **Current branch (ready for PR):** `worktree-fix+dark-theme-enrolment-icons`, off `main` (`f52e845`, after PR #21) — course-listing enrolment-method icons (#138), footer "Show footer" help-button `?` icon (#139), the Messaging drawer full dark surface (#140), and the Calendar month-view day-cell hover (#141). All user-verified on Moodle Cloud; dark presets only; no new tokens, presets, or controls. See the #138–#142 bullets below + `docs/PROJECT-TRACKER.md`. Also: the Move-block YUI dialog dark surface (#142), the focused content-link "dark text on the light focus box" fix (#143), and the User-report teacher route (#144). (A grader-report "Overall average"/heading/category light-cell fix (#145) is in progress on this branch.)
