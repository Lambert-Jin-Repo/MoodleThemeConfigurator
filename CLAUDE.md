...
- **Newly reported (separate fix, in progress):** on dark themes the pre-existing page-wrapper rule (`#page, #page-wrapper, … { background-color: ${tokens.pageBg} !important }`, `scss-generator.ts:482`) covers Moodle's `body`-level background image, hiding it. NOT a quiz-edit regression. Plan: make the page wrapper transparent with `pageBg` as a `body`-level fallback; cards/nav stay opaque.
