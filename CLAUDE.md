...
- **Current branch (ready for PR):** `fix/dark-theme-grade-report-contrast`, off `main` (`fa8611b`, after PR #20) — student User report grade table (#136) + gradebook cell-actions three-dot icon (#137). User-verified on Moodle Cloud; dark presets only; no new tokens, presets, or controls. See the #136–#137 bullet below + `docs/PROJECT-TRACKER.md`.
- **Previous branch (merged):** `worktree-fix+dark-theme-quiz-radio-contrast` → PR #20 into `main` — quiz answer-radio selection (#133), tooltip text (#134), icon-button hover (#135).
- **Earlier branches (merged):** quiz-edit #113–116 (`worktree-fix+dark-theme-quiz-edit-contrast`) and background image #117 (`worktree-fix+dark-theme-bg-image-overlay`), plus #118–132 across the qbank/quiz-view/gradebook/timer/formulation branches — all merged into `main` via PRs #11–19.
